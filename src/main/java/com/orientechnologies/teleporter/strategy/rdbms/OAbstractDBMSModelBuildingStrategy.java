...
public abstract class OAbstractDBMSModelBuildingStrategy implements OWorkflowStrategy {
  ...
  public ODocument executeStrategy(
    ...
    if (keepVerticesCoordinates && migrationConfig != null) {
      // update vertices contained in the new just built config adding the correspondent coordinates
      // from the old configuration (if not null!)
  ...
  public void addCoordinatesFromOldConfiguration(
      OConfiguration newConfig, OConfiguration oldConfig) {
    ...
    for (OConfiguredVertexClass currVertexClass : oldConfig.getConfiguredVertices()) {
      ...
      OConfiguredVertexClass newVertexClass =
          newConfig.getVertexClassByName(currVertexClass.getName());
