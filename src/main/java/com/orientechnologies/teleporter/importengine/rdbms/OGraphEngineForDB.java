...
public class OGraphEngineForDB {
  ...
  public boolean alreadyFullImportedInOrient(OrientBaseGraph orientGraph, ResultSet record, OVertexType vertexType,
      Set<String> propertiesOfIndex, OTeleporterContext context) throws SQLException {
  ...
  public Vertex upsertVisitedVertex(OrientBaseGraph orientGraph, ResultSet record, OVertexType vertexType,
      Set<String> propertiesOfIndex, OTeleporterContext context) {
    ...
    try {
      ...
      for (OModelProperty currentProperty : vertexType.getAllProperties()) {
        ...
        currentPropertyType = context.getDataTypeHandler()
            .resolveType(currentProperty.getPropertyType().toLowerCase(Locale.ENGLISH), context).toString();
        String currentOriginalType = currentProperty.getPropertyType();

        try {
          extractPropertiesFromRecord(record, properties, currentPropertyType, currentPropertyName, currentOriginalType,
              vertexType);
        } catch (Exception e) {
          String mess =
            ...
            if (equalProperties) {
              ...
              for (String propertyName : vertex.getPropertyKeys()) {
                if (!(vertex.getProperty(propertyName) == null && properties.get(propertyName) == null)) {
                  currentPropertyType = context.getDataTypeHandler()
                      .resolveType(vertexType.getPropertyByName(propertyName).getPropertyType().toLowerCase(Locale.ENGLISH),
                          context).toString();
  ...
  private void extractPropertiesFromRecord(ResultSet record, Map<String, Object> properties, String currentPropertyType,
   ...
   * @param edgeType            type of the OEdgeType present between the two OVertexType, used as label during the insert of the
   *                            edge in the graph
   *                            <p>
  ...
  public OrientVertex upsertReachedVertexWithEdge(OrientBaseGraph orientGraph, ResultSet foreignRecord, ORelationship relation,
      OrientVertex currentOutVertex, OVertexType currentInVertexType, String edgeType, OTeleporterContext context)
      throws SQLException {
    ...
    try {
      ...
      if (ok) {
        ...
        if (currentInVertex == null) {
          ...
          currentInVertex = this.addVertexToGraph(orientGraph, classAndClusterName, partialProperties, context);
          statistics.orientAddedVertices++;
  ...
  public void upsertEdge(OrientBaseGraph orientGraph, OrientVertex currentOutVertex, OrientVertex currentInVertex, String edgeType,
      Map<String, Object> properties, String direction, OTeleporterContext context) {
      ...
      if (it.hasNext()) {
        ...
        if (edgeAlreadyPresent) {
          ...
          if (direction != null && direction.equals("direct")) {
            edge = this.addEdgeToGraph(orientGraph, null, currentOutVertex, currentInVertex, edgeType, context);
          } else if (direction != null && direction.equals("inverse")) {
            edge = this.addEdgeToGraph(orientGraph, null, currentInVertex, currentOutVertex, edgeType, context);
          }
          this.setElementProperties(edge, properties, context);
  ...
  public void upsertAggregatorEdge(OrientBaseGraph orientGraph, ResultSet jointTableRecord, OEntity joinTable,
      OAggregatorEdge aggregatorEdge, OTeleporterContext context) throws SQLException {
    ...
    try {
      ...
      for (OAttribute foreignKeyAttribute : relationship1.getForeignKey().getInvolvedAttributes()) {
        keysOutVertex[index] = this.mapper.getPropertyNameByEntityAndAttribute(relationship1.getParentEntity(),
            relationship1.getPrimaryKey().getInvolvedAttributes().get(index).getName());
      ...
      for (OAttribute foreignKeyAttribute : relationship2.getForeignKey().getInvolvedAttributes()) {
        keysInVertex[index] = this.mapper.getPropertyNameByEntityAndAttribute(relationship2.getParentEntity(),
            relationship2.getPrimaryKey().getInvolvedAttributes().get(index).getName());
      ...
      for (OModelProperty currentProperty : edgeType.getAllProperties()) {
        ...
        String currentPropertyType = context.getDataTypeHandler()
            .resolveType(currentProperty.getPropertyType().toLowerCase(Locale.ENGLISH), context).toString();
        String currentOriginalType = currentProperty.getPropertyType();
        OVertexType joinVertexType = this.mapper.getJoinVertexTypeByAggregatorEdge(edgeType.getName());

        try {
          extractPropertiesFromRecord(jointTableRecord, properties, currentPropertyType, currentPropertyName, currentOriginalType,
              joinVertexType);
        } catch (Exception e) {
  ...
  private OrientEdge addEdgeToGraph(OrientBaseGraph orientGraph, Object id, OrientVertex currentOutVertex,
      OrientVertex currentInVertex, String edgeType, OTeleporterContext context) {
  ...
  private void setElementProperties(OrientElement element, Map<String, Object> properties, OTeleporterContext context) {
