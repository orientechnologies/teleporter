	...
	public void executeImport(String driver, String uri, String username, String password, String outOrientGraphUri, OSource2GraphMapper genericMapper,  OTeleporterContext context) {
		...
		try {
			...
			for(OHierarchicalBag bag: mapper.getDataBaseSchema().getHierarchicalBags()) {
				...
				case "table-per-hierarchy": super.tablePerHierarchyImport(bag, mapper, dbQueryEngine, graphDBCommandEngine, orientGraph, context);
				break;

				case "table-per-type": super.tablePerTypeImport(bag, mapper, dbQueryEngine, graphDBCommandEngine, orientGraph, context);
				break;

				case "table-per-concrete-type": super.tablePerConcreteTypeImport(bag, mapper, dbQueryEngine, graphDBCommandEngine, orientGraph, context);
				break;
			...
			for(OEntity entity: mapper.getDataBaseSchema().getEntities()) {
				...
				if(entity.isJoinEntityDim2() && entity.getHierarchicalBag() == null) {
					...
					while(records.next()) {
						currentRecord = records;
						graphDBCommandEngine.upsertAggregatorEdge(orientGraph, currentRecord, entity, aggregatorEdge, context);
