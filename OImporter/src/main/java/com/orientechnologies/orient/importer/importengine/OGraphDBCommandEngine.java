...
package com.orientechnologies.orient.importer.importengine;
...
public class OGraphDBCommandEngine {
  ...
  public Vertex upsertVisitedVertex(ResultSet record, OVertexType vertexType, ONameResolver nameResolver) throws SQLException {
    ...
    vertex.save();
    OLogManager.instance().info(this, "New vertex inserted (all props setted): %s\n", vertex.toString());
   ...
   * - if the vertex is already present in the graph no update is performed, neither on reache-vertex neither on the relative edge
   * @throws SQLException 
   */
  ...
  public Vertex upsertReachedVertexWithEdge(ResultSet foreignRecord, ORelationship relation, Vertex currentOutVertex, OVertexType currentInVertexType, String edgeType, ONameResolver nameResolver) throws SQLException {
    ...
    if(ok) {
      ...
      edge.save();
      OLogManager.instance().info(this, "New edge inserted: %s\n", edge.toString());
